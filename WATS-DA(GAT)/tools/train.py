...
parser.add_argument('--cfg', type=str, default='/media/w/719A549756118C56/HGG/WATS-DA(GAT)/experiments/siamgat_googlenet/config.yaml',
...
def build_opt_lr(model, current_epoch=0):
    ...
    # trainable_params += [{'params': model.diffusion.parameters(),
    #                       'lr': cfg.TRAIN.BASE_LR_d}]
    
    trainable_params += [{'params': model.align_track.parameters(),
                          'lr': cfg.TRAIN.BASE_LR_d}]
...
def train(source_loader, target_loader, model, optimizer, tb_writer,lr_scheduler, Disc, optimizer_D):
    ...
    for idx in range(cfg.TRAIN.EPOCH*num_per_epoch):
        ...
        zf_up_t = interp(zf)
        xf_up_t = interp(xf)
        D_out_z = Disc(F.softmax(zf_up_t, dim=1)).sum(0) / 3.
        D_out_x = Disc(F.softmax(xf_up_t, dim=1)).sum(0) / 3.
        ...
        if is_valid_number(loss_adv.data.item()):
            ...
            loss_adv.requires_grad_().backward()
        ...
        for param in Disc.parameters():
            param.requires_grad = True 
        zf_up_t = zf_up_t.detach()
        xf_up_t = xf_up_t.detach()
        D_out_1 = Disc(F.softmax(zf_up_t, dim=1)).sum(0) / 3.
        D_out_2 = Disc(F.softmax(xf_up_t, dim=1)).sum(0) / 3.
        ...
        zf_up_s = zf_up_s.detach() 
        xf_up_s = xf_up_s.detach()
        D_out_1 = Disc(F.softmax(zf_up_s, dim=1)).sum(0) / 3.
        D_out_2 = Disc(F.softmax(xf_up_s, dim=1)).sum(0) / 3.
        D_source_label = torch.FloatTensor(D_out_z.data.size()).fill_(source_label)
...
def main():
    ...
    # start training
    train(source_loader, target_loader, model, optimizer, tb_writer,lr_scheduler, model_Disc, optimizer_D)
