...
# Modified from UDAT, https://github.com/vision4robotics/UDAT
...
class SubDataset(object):
    ...
    def get_image_anno(self, video, track, frame):
        if self.name == "WATB400_1"or self.name=="WATB400_2"or self.name=="Wildlife2024_4_1"or self.name=="Wildlife2024_4_2":
            image_path = os.path.join(self.root, video, track, "000000.{:02d}.x.jpg".format(frame))
            frame = "{:06d}".format(frame)
        else:
            # frame = int(frame)  # 将frame转换为整数
            frame = "{:06d}".format(frame)
...
class BANDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if template_image is None or search_image is None:
            print(f'None image in {template[0]} or {search[0]}')
        # get bounding box
