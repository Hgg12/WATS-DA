...
from siamban.utils.distributed import dist_init, DistModule, reduce_gradients, average_reduce, get_rank, get_world_size
...
parser.add_argument('--cfg', type=str, default='configs/config.yaml', help='configuration of tracking')
...
def build_data_loader(domain):
    ...
    train_loader = DataLoader(train_dataset, batch_size=cfg.TRAIN.BATCH_SIZE, 
                              num_workers=cfg.TRAIN.NUM_WORKERS, pin_memory=True, sampler=train_sampler)
    return train_loader
...
def train(source_loader, target_loader, model, optimizer, lr_scheduler, tb_writer, Disc, optimizer_D):
    ...
    num_per_epoch = len(target_loader.dataset) // cfg.TRAIN.EPOCH // (cfg.TRAIN.BATCH_SIZE * world_size)
...
def main():
    ...
    if args.model == 'WATS-DA':
        cfg.DATASET.VIDEOS_PER_EPOCH = cfg.DATASET.VIDEOS_PER_EPOCH_B
        
    else:
        raise ValueError('Unknown model name, please select from []')

    cfg.TRAIN.LOG_DIR += str(args.model)
    cfg.TRAIN.SNAPSHOT_DIR += str(args.model)
