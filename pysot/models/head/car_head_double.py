...
class CARHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        #     padding=1
        # )
        self.cls_logits_linear = nn.Linear(in_channels, num_classes)
        ...
        for modules in [self.cls_tower, self.bbox_tower,
                        self.cls_logits_linear, self.bbox_pred,
                        self.centerness]:
        ...
        torch.nn.init.constant_(self.cls_logits_linear.bias, bias_value)

    def forward(self, x):
        cls_tower = self.cls_tower(x)

        logits_in = cls_tower.permute(0, 2, 3, 1)
        logits = self.cls_logits_linear(logits_in)
